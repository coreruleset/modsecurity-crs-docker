...
| HTTP3 | A string value indicating whether HTTP/3 (QUIC) should be enabled on `SSL_PORT`, in addition to HTTP/1.1 and HTTP/2 over TCP (Allowed values: `on`, `off`. Default: `off`). QUIC runs over UDP, so the container's `SSL_PORT` must also be published as a UDP port (e.g. `-p 8443:8443/udp`) for clients to reach it. |
