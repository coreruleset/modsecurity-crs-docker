...
| NGINX_WORKER_PROCESSES | Controls nginx's [`worker_processes`](https://nginx.org/en/docs/ngx_core_module.html#worker_processes) directive. Set to a fixed number (e.g. `1`) to serialize log writes across workers, avoiding out-of-order log entries when relying on log markers for testing. (Default: `auto`) |
